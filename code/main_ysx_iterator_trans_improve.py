...
def initParam(arg, p1, p2):
    ...
    if lb_flag:
        ...
        if int(flag) == 1:
            ...
            train_file = '../data/extract_remain_data/2016_1_1/train.csv'
            test_file = '../data/extract_remain_data/2016_1_1/test.csv'
            # train_file = '../data/train_partial2016.csv'
            # test_file = '../data/test_partial2016.csv'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        arg = True
        device = torch.device("cuda:" + sys.argv[1]) if torch.cuda.is_available() else torch.device("cpu")
        print('CUDA' + sys.argv[1])
        print('按照参数设置配置')
        pass
    else:
        print('没有传参，按照手工进行设置')
    ...
    if int(input_flag) == 0:
        if not arg:
            ...
            initParam(arg, sys.argv[3], sys.argv[4])
            pass
        pass
    ...
    # 获取参数输入的pos_weight
    pos_weight_num = float(sys.argv[6])
